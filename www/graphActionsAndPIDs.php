...
//      die("action query Failed\n");
    ...
    $devCnt = mysqli_num_rows($actionResult);
    fwrite($fr, $devCnt." action rows retrieved\n");
    echo $devCnt." action rows retrieved\n";
    $actionObjCnt = 0;
    do
    {
      fwrite($fr, "actionObjCnt = $actionObjCnt\n");        
      $actionObjCnt++;
      
      $actionObj = mysqli_fetch_object($actionResult);     // Get and store Action data to graph database table
      if($actionObj === NULL)
      {
        fwrite($fr, "NULL\n");
        break;
      ...
      $tIn = rtrim($showActionStatus.$actionObj->id); 
      $tOut = trim($out);
      fwrite($fr, "\$in = $tIn, \$out = $out \n");

      if($tOut === "Invalid Command")
      {
        fwrite($fr, "Invalid Command\n");
        continue;
      }
